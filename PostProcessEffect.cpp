...
bool PostProcessEffect::InitAll(ID3D11Device* device)
{
	...
	// 创建顶点着色器
	HR(pImpl->m_pEffectHelper->CreateShaderFromFile("MinimapVS", L"HLSL\\Minimap_VS.cso", device));
	HR(pImpl->m_pEffectHelper->CreateShaderFromFile("ScreenFadeVS", L"HLSL\\ScreenFade_VS.cso", device));

	// 创建像素着色器
	HR(pImpl->m_pEffectHelper->CreateShaderFromFile("MinimapPS", L"HLSL\\Minimap_PS.cso", device));
	HR(pImpl->m_pEffectHelper->CreateShaderFromFile("ScreenFadePS", L"HLSL\\ScreenFade_PS.cso", device));
