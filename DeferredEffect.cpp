...
bool DeferredEffect::InitAll(ID3D11Device* device)
{
	...
	HR(pImpl->m_pEffectHelper->CreateShaderFromFile("FullScreenTriangleVS", L"HLSL\\36\\FullScreenTriangle.hlsl",
		device, "FullScreenTriangleVS", "vs_5_0", defines));
	HR(pImpl->m_pEffectHelper->CreateShaderFromFile("GeometryVS", L"HLSL\\36\\GBuffer.hlsl",
	...
	while (msaaSamples <= 8)
	{
		...
		HR(pImpl->m_pEffectHelper->CreateShaderFromFile(shaderNames[1], L"HLSL\\36\\GBuffer.hlsl",
			device, "RequiresPerSampleShadingPS", "ps_5_0", defines));
		HR(pImpl->m_pEffectHelper->CreateShaderFromFile(shaderNames[2], L"HLSL\\36\\BasicDeferred.hlsl",
			device, "BasicDeferredPS", "ps_5_0", defines));
		HR(pImpl->m_pEffectHelper->CreateShaderFromFile(shaderNames[3], L"HLSL\\36\\BasicDeferred.hlsl",
			device, "BasicDeferredPerSamplePS", "ps_5_0", defines));
		HR(pImpl->m_pEffectHelper->CreateShaderFromFile(shaderNames[4], L"HLSL\\36\\GBuffer.hlsl",
	...
	D3D11_VIEWPORT viewport)
{
	...
	deviceContext->PSSetShaderResources(pImpl->m_pEffectHelper->MapShaderResourceSlot("g_GBufferTextures[2]"), 1, &posZGradGBuffer);
}

void DeferredEffect::ComputeLightingDefault(ID3D11DeviceContext* deviceContext,
