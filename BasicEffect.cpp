...
public:
	...
	ComPtr<ID3D11InputLayout> m_pVertexPosSizeLayout;
...
bool BasicEffect::InitAll(ID3D11Device* device)
{
	...
	HR(device->CreateInputLayout(VertexPosNormalTex::inputLayout, ARRAYSIZE(VertexPosNormalTex::inputLayout), blob->GetBufferPointer(),
		blob->GetBufferSize(), pImpl->m_pVertexPosNormalTexLayot.GetAddressOf()));
	...
	D3D11SetDebugObjectName(pImpl->m_pVertexPosNormalTexLayot.Get(), "VertexPosNormalTexLayot");
	...
	D3D11SetDebugObjectName(pImpl->m_pBasicVS.Get(), "m_pBasicVS");
	D3D11SetDebugObjectName(pImpl->m_pBasicPS.Get(), "m_pBasicPS");
	D3D11SetDebugObjectName(pImpl->m_pBillboardVS.Get(), "m_pBillboardVS");
	D3D11SetDebugObjectName(pImpl->m_pBillboardGS.Get(), "m_pBillboardGS");
	D3D11SetDebugObjectName(pImpl->m_pBillboardPS.Get(), "BillboardPS");
	return true;
}
...
void BasicEffect::SetRenderDefault(ID3D11DeviceContext* deviceContext)
{
	...
	deviceContext->IASetInputLayout(pImpl->m_pVertexPosNormalTexLayot.Get());
...
void BasicEffect::SetRenderBillboard(ID3D11DeviceContext* deviceContext, bool enableAlphaToCoverage)
{
	...
	deviceContext->PSSetSamplers(0, 1, RenderStates::SSLinearWrap.GetAddressOf());
    deviceContext->OMSetDepthStencilState(nullptr, 0);
	deviceContext->OMSetBlendState((enableAlphaToCoverage ? RenderStates::BSAlphaToCoverage.Get() : nullptr), nullptr, 0xFFFFFFFF);
}
