...
float2 VarianceShadowPS(float4 posH : SV_Position, float2 texCoord : TEXCOORD) : SV_Target
{
    uint2 coords = uint2(posH.xy);
    float2 depth;
...
float2 EVSM2CompPS(float4 posH : SV_Position, float2 texCoord : TEXCOORD) : SV_Target
{
    uint2 coords = uint2(posH.xy);
...
float4 GaussianBlurXPS(float4 posH : SV_Position, float2 texCoord : TEXCOORD) : SV_Target
{
    float4 depths = 0.0f;
    [unroll]
...
float4 GaussianBlurYPS(float4 posH : SV_Position, float2 texCoord : TEXCOORD) : SV_Target
{
    float4 depths = 0.0f;
    [unroll]
...
float LogGaussianBlurPS(float4 posH : SV_Position, float2 texCoord : TEXCOORD) : SV_Target
{
