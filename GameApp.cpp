...
void GameApp::UpdateScene(float dt)
{
	...
	else if (m_KeyboardTracker.IsKeyPressed(Keyboard::D2) && m_CurrMode != ShowMode::FireAnim)
	{
		...
		m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pFireAnimes[0].GetAddressOf());
	}
	...
	if (m_CurrMode == ShowMode::WoodCrate)
	{
		...
		//m_VSConstantBuffer.rotation = XMMatrixTranslation(-0.5f, -0.5f, 0.0f) * XMMatrixRotationZ(phi) * XMMatrixTranslation(0.5f, 0.5f, 0.0f);
		m_VSConstantBuffer.rotation = XMMatrixRotationZ(phi);
	...
	else if (m_CurrMode == ShowMode::FireAnim)
	{
		...
		if (totDeltaTime > 1.0f / 60)
		{
			...
			m_pd3dImmediateContext->PSSetShaderResources(0, 1, m_pFireAnimes[m_CurrFrame].GetAddressOf());
		}
	}
}
...
bool GameApp::InitResource()
{
	...
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\flarealpha.dds", nullptr, m_pFire.GetAddressOf()));
	HR(CreateDDSTextureFromFile(m_pd3dDevice.Get(), L"..\\Texture\\flare.dds", nullptr, m_pWoodCrate.GetAddressOf()));
	WCHAR strFile[40];
	m_pFireAnimes.resize(120);
	for (int i = 1; i <= 120; ++i)
	{
		wsprintf(strFile, L"..\\Texture\\FireAnim\\Fire%03d.bmp", i);
		HR(CreateWICTextureFromFile(m_pd3dDevice.Get(), strFile, nullptr, m_pFireAnimes[static_cast<size_t>(i) - 1].GetAddressOf()));
	}
	...
	m_VSConstantBuffer.rotation = XMMatrixIdentity();
