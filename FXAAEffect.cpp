...
bool FXAAEffect::InitAll(ID3D11Device* device)
{
	...
	pImpl->m_pEffectHelper->CreateShaderFromFile("FullScreenTriangleTexcoordVS", L"HLSL\\36\\FXAA.hlsl", device,
	...
	for (auto str : numStrs)
	{
		...
		pImpl->m_pEffectHelper->CreateShaderFromFile(psName, L"HLSL\\36\\FXAA.hlsl", device,
			"PS", "ps_5_0", defines);
...
void FXAAEffect::RenderFXAA(ID3D11DeviceContext* deviceContext, ID3D11ShaderResourceView* input, ID3D11RenderTargetView* output, const D3D11_VIEWPORT& vp)
{
	...
	input = nullptr;
	deviceContext->PSSetShaderResources(pImpl->m_pEffectHelper->MapShaderResourceSlot("g_TextureInput"), 1, &input);
